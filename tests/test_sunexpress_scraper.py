...
from bot import sunexpress_scraper
...
async def test_fetch_price_exception_returns_none(monkeypatch, caplog):
    ...
    monkeypatch.setattr(sunexpress_scraper, "async_playwright", lambda: pw_cm)
    monkeypatch.setattr(sunexpress_scraper, "stealth_async", AsyncMock())

    with caplog.at_level("WARNING", logger="bot.sunexpress_scraper"):
        result = await sunexpress_scraper.fetch_price("IST", "LHR", "2026-06-01", "2026-06-10")
